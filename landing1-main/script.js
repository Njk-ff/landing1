...
// я забыл как делаються слайдеры (´。＿。｀)

const slidan = document.querySelector(".slides")
const butLeft = document.querySelector(".slides__but-r")
const butRight = document.querySelector(".slides__but-l")

butRight.addEventListener("click", function (e) {
    let a = window.getComputedStyle(slidan).left
    a = parseInt(a, 10);
    if (a > -1300) {
        a = a - 300
        console.log(a)
        slidan.style.left = a + "px" 
    }


});

butLeft.addEventListener("click", function (e) {
    let a = window.getComputedStyle(slidan).left
    a = parseInt(a, 10);
    if (a < 0) {
        a = a + 300
